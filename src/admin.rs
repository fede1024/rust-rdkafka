...
impl NativeEvent {
    ...
    pub fn ptr(&self) -> *mut RDKafkaEvent {
        self.ptr
    }
}
...
fn check_rdkafka_invalid_arg(res: RDKafkaRespErr, err_buf: &ErrBuf) -> KafkaResult<()> {
    match res.into() {
        ...
        RDKafkaError::InvalidArgument => {
            ...
            Err(KafkaError::AdminOp(msg))
        }
        res => Err(KafkaError::AdminOp(format!(
...
impl<'a> NewTopic<'a> {
    ...
    fn to_native(&self, err_buf: &mut ErrBuf) -> KafkaResult<NativeNewTopic> {
        ...
        let repl = match self.replication {
            ...
            TopicReplication::Variable(partitions) => {
                if partitions.len() as i32 != self.num_partitions {
                    return Err(KafkaError::AdminOp(format!(
        ...
        if topic.is_null() {
            return Err(KafkaError::AdminOp(err_buf.to_string()));
        }
...
impl NativeNewPartitions {
    ...
    pub fn ptr(&self) -> *mut RDKafkaNewPartitions {
        self.ptr
    }
}
...
impl NativeConfigResource {
    ...
    pub fn ptr(&self) -> *mut RDKafkaConfigResource {
        self.ptr
    }
}
    ...
    match typ {
        ...
        RDKafkaResourceType::RD_KAFKA_RESOURCE_BROKER => {
            ...
            match name.parse::<i32>() {
                ...
                Err(_) => Err(KafkaError::AdminOp(format!(
                    "bogus broker ID in kafka response: {}",
                    name
                ))),
            }
        }
        _ => Err(KafkaError::AdminOp(format!(
