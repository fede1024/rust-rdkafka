...
impl Producer {
    ...
    pub fn send_copy<P, K>(&self, topic: &Topic, payload: Option<&P>, key: Option<&K>) -> KafkaResult<DeliveryFuture>
        where K: ToBytes,
              P: ToBytes {
        self._send_copy(topic, None, payload.map(P::to_bytes), key.map(K::to_bytes))
    }

    /// Sends a copy of the message and key provided with the partition specified. Returns a `DeliveryFuture` or an error.
    pub fn send_copy_to_partition<P, K>(&self, partition: i32, topic: &Topic, payload: Option<&P>, key: Option<&K>) -> KafkaResult<DeliveryFuture>
        where K: ToBytes,
              P: ToBytes {
        self._send_copy(topic, Some(partition), payload.map(P::to_bytes), key.map(K::to_bytes))
    }
