...
fn test_topics() {
    ...
    // be deleted.
    {
        ...
        let topic1 = NewTopic::new(&name1, 1, TopicReplication::Fixed(1)).set("flush.ms", "1234");
        let topic2 = NewTopic {
            name: &name2,
            num_partitions: 3,
        ...
        let expected_entry1 = ConfigEntry {
            name: "flush.ms".into(),
        ...
        let expected_entry2 = ConfigEntry {
            name: "flush.ms".into(),
            value: Some(format!("{}", i64::max_value())),
            source: ConfigSource::Default,
            is_read_only: false,
            is_default: true,
            is_sensitive: false,
        };
        assert_eq!(Some(&expected_entry1), config1.get("flush.ms"));
        assert_eq!(Some(&expected_entry2), config2.get("flush.ms"));
        ...
        assert_eq!(Some(&&expected_entry1), config_entries1.get("flush.ms"));
        assert_eq!(Some(&&expected_entry2), config_entries2.get("flush.ms"));
...
fn test_configs() {
    ...
    (|| {
        ...
        let expected_entry = &ConfigEntry {
